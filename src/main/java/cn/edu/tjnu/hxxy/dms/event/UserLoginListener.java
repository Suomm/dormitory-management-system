...
import static cn.edu.tjnu.hxxy.dms.constant.RabbitConst.*;
import static cn.edu.tjnu.hxxy.dms.constant.RedisConst.PREFIX_USER_LOCKED;
...
public class UserLoginListener {
    ...
    public UserLoginListener(RedisUtil redisUtil,
     ...
     */
    @EventListener
    public void badCredentials(@NotNull AuthenticationFailureBadCredentialsEvent event) {
        // 获取登陆的学号
        ...
        // 根据键值获取对应的登录次数
        int limit = redisUtil.get(PREFIX_USER_LOCKED.concat(username), 1);
        if (limit < MAX_LOGIN_COUNT) {
            // 登录次数小于极限，每次登陆失败都要记录，记录三十分钟之后失效
            redisUtil.set(PREFIX_USER_LOCKED.concat(username), limit + 1, 30, TimeUnit.MINUTES);
        } else if (limit == MAX_LOGIN_COUNT){
            // 登录次数等于极限值时锁定用户
            setUserNonLocked(username, false);
            // 取消缓存失效时间，手动删除缓存，为了时间一致性
            redisUtil.set(PREFIX_USER_LOCKED.concat(username), limit + 1);
     ...
     */
    @EventListener
    public void success(@NotNull AuthenticationSuccessEvent event) {
        // 获取操作的用户名
        String username = event.getAuthentication().getPrincipal().toString();
        // 判断是否含有登陆失败的记录键，有的话删除对应的键
        if (redisUtil.hasKey(PREFIX_USER_LOCKED.concat(username))) {
            boolean remove = redisUtil.delete(PREFIX_USER_LOCKED.concat(username));
    ...
    public void unlocked(String username) {
        redisUtil.delete(PREFIX_USER_LOCKED.concat(username));
