...
import cn.edu.tjnu.hxxy.dms.exception.InvalidOrOverdueException;
import cn.edu.tjnu.hxxy.dms.exception.MissingAttributeException;
import cn.edu.tjnu.hxxy.dms.entity.Captcha;
import cn.edu.tjnu.hxxy.dms.entity.User;
import cn.edu.tjnu.hxxy.dms.service.UserService;
import cn.edu.tjnu.hxxy.dms.util.MD5Util;
import cn.edu.tjnu.hxxy.dms.validation.constraints.Id;
import cn.edu.tjnu.hxxy.dms.validation.constraints.Password;
...
public class UserController extends BaseController<UserService> {
    ...
    public boolean register(@ApiParam(value = "用户信息", required = true) @Validated User user,
        ...
        // 获取缓存中的验证码对象
        Captcha captcha = redisUtil.get(key);
        // 校验邮箱验证码
        CaptchaUtil.checkCaptcha(captcha, CaptchaUtil.getCaptcha(emailCode));
        // 检验部门的邀请码是否正确
        if (!validateCode.equals(redisUtil.get(KEY_CAPTCHA))) {
    ...
    public void forgetPassword(@PathVariable @ApiParam(value = "用户学号", required = true) @Id String id) {
        ...
        // 生成数字签名，加密 KEY
        String digitalSignature = MD5Util.encode(key).toUpperCase();
        // 存入 Redis 缓存，设置十分钟后过期
        redisUtil.set(PREFIX_RESET_PASSWORD.concat(user.getId()), new Captcha(secretKey, outDate), 10, TimeUnit.MINUTES);
        // 拼接请求路径
        String resetPassHref = ServletUtil.getBasePath() + "/reset-password.html" + "?id=" + user.getId() + "&sid=" + digitalSignature;
    ...
    public boolean checkup(@ApiParam(value = "密匙", required = true) @NotBlank String id,
        ...
        // 获取之前存入缓存的验证链接
        Captcha captcha = redisUtil.get(PREFIX_RESET_PASSWORD.concat(id));
        // 没有输入自己的学号以及链接过期了
        return captcha != null && captcha.getOutDate() > System.currentTimeMillis() &&
                MD5Util.matches(id + DELIMITER + captcha.getOutDate() + DELIMITER + captcha.getCode(), sid);
    }
    ...
    public void getCaptcha(@ApiParam(value = "学号", required = true) @Id String id,
        ...
        // 生成六位、十分钟后过期的验证码
        Captcha code = CaptchaUtil.getCaptcha(6, 10L * 60 * 1000);
        // 编辑要发送的邮件内容
        String emailContent = "您的验证码为" + code.getCode() + "\t有效期为十分钟";
        // 发送验证码邮件
        sendEmail("验证码", emailContent, email);
        // 将验证码内容存入 Redis 缓存中，并设置十分钟后过期
        redisUtil.set(PREFIX_USER_CAPTCHA.concat(id), code, 10, TimeUnit.MINUTES);
    }
    ...
    public boolean changeEmail(@PathVariable @Id String id,
        ...
        // 获取缓存中的验证码对象
        Captcha captcha = redisUtil.get(PREFIX_USER_CAPTCHA.concat(id));
        // 校验邮箱验证码
        CaptchaUtil.checkCaptcha(captcha, CaptchaUtil.getCaptcha(emailCode));
        // 修改邮箱成功后删除验证码
        if (service.changeEmail(id, oldEmail, newEmail)) {
            redisUtil.delete(PREFIX_USER_CAPTCHA.concat(id));
            return true;
        }
        return false;
    }
